...
def _extract_distortion_payload(
    thd_data: Any,
    rtings_metrics: Optional[Mapping[str, Any]] = None,
) -> dict:
    payload: dict = {}
    ...
    if rtings_metrics:
        # BUG FIX: added "is_ba_iem" to the list of keys copied from rtings_metrics.
        # Previously it was missing, so flag_impedance_interaction() always received
        # is_ba_iem=False and the impedance flag never fired for BA IEMs.
        for key in (
            "thd",
            "thd_plus_n",
            "imd",
...
def run_evaluation_pipeline(
    ...
    # Peakiness measures roughness of the FR *error* relative to the Harman target,
    # not absolute SPL. Using (final_mags - target_erb) removes the broad-stroke
    # slope and DC offset so only narrow resonances and local jaggedness remain.
    peakiness     = calculate_peakiness_metric(final_mags - target_erb, freqs=erb_freqs)

    distortion_payload = _extract_distortion_payload(thd_data, rtings_metrics=rtings_metrics)

    # thd_plus_n takes priority over thd when both are present
    thd_measurements = (
        distortion_payload.get("thd_plus_n")
        if distortion_payload.get("thd_plus_n") is not None
        else distortion_payload.get("thd")
    )
    ...
    # is_ba_iem is now correctly propagated from rtings_metrics (Bug #2 fix)
    ...
    e_unc = calculate_uncertainty(
        variance, n, thd_available, match_available,
        headphone_name=name, source_names=source_labels,
    )
    if impedance_flag:
        e_unc += 0.15
    ...
    score = (
        calculate_score(e_total, w_conf, price, sensitivity_db_mw=sensitivity_db_mw)
    ...
    if score is not None:
        ci = calculate_score_confidence_interval(
            e_total=e_total, price=price, e_unc=e_unc,
            variance_array=variance, n_sources=n,
            thd_available=thd_available, match_available=match_available,
            e_fr_profile=e_fr_profile, distortion_profile=distortion_profile,
            e_match=e_match, sensitivity_db_mw=sensitivity_db_mw,
            headphone_name=name, source_names=source_labels,
        )
