...
def _try_fetch(path_prefix, name, nested, sources, label):
    ...
    if l_text and r_text:
        ...
        if left and right:
            ...
            r_al    = ir(left[0])
            sources.append({'freqs': left[0], 'mags': (left[1] + r_al) / 2,
                            'left_mags': left[1], 'right_mags': r_al,
                            'source': label})
            return
    ...
    if m_text:
        ...
        if mono:
            sources.append({'freqs': mono[0], 'mags': mono[1],
                            'left_mags': mono[1], 'right_mags': mono[1],
                            'source': label})
...
def fetch_autoeq_data(headphone_name, library_entry=None):
    ...
    if library_entry:
        ...
        _try_fetch("oratory1990/data/over-ear", headphone_name, False, sources, "oratory1990/over-ear")
